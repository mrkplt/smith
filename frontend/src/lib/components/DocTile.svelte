...
<Card class="bg-slate-900/40 border-gray-800 hover:border-[#86BC25]/50 transition-all backdrop-blur-sm p-4 {doc.status === 'archived' ? 'opacity-60' : ''} rounded-none">
  <div class="flex flex-col h-full gap-3">
    <div class="flex justify-between items-start gap-2">
      ...
      <Badge color="gray" class="uppercase text-[10px] font-bold px-2 py-0.5 rounded-none {doc.status === 'active' ? 'bg-[#86BC25] text-black' : 'bg-slate-800 text-gray-400'}">
        {doc.status || 'active'}
      </Badge>
    </div>
    ...
    <div class="mt-auto pt-3 border-t border-gray-800/50 flex gap-2">
      ...
      <Button color="alternative" size="xs" class="px-2 rounded-none bg-[#86BC25] text-black hover:bg-[#6b961d]" onclick={() => onBuild(doc.id)} title="Build">
