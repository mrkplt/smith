<script lang="ts">
    ...
    async function handleSubmit(e: Event) {
        e.preventDefault();
        if (!content.trim() || chatSession.isStreaming || disabled) return;
...
<form onsubmit={handleSubmit} class="p-4 border-t border-gray-800 bg-gray-900">
    <div class="relative flex items-center">
        <textarea
            ...
            rows="1"
            disabled={chatSession.isStreaming || disabled}
        ></textarea>
        <button
            type="submit"
            disabled={!content.trim() || chatSession.isStreaming || disabled}
