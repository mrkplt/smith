...
| M-009 | E2E | Ingress mode coverage (GitHub, PRD, interactive) | `./scripts/test/e2e-ingress-modes.sh` | `smith` ingress commands create loops and each created loop resolves to synced state with source traceability (`source_type`, `source_ref`) |
| M-010 | E2E | Environment mode coverage (preset, mise, image, dockerfile) | `./scripts/test/e2e-environment-modes.sh` | `smith loop create` environment flags map to deterministic resolved mode metadata |
