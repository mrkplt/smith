...
Local note: `make deploy-local` builds the Smith images locally and imports them into the k3d cluster before Helm runs, so the default `ghcr.io/smith/*:v0.4.1` refs resolve without a remote registry pull.
...
Image tag defaults:
- `core.image.tag: v0.4.1`
- `api.image.tag: v0.4.1`
- `console.image.tag: v0.4.1`
- `chat.image.tag: v0.4.1`
