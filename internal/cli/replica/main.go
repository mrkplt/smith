...
func Run() int {
	...
	if desiredState == model.LoopStateSynced {
		...
		commitSummary, prTitle, prBody := buildCompletionTraceability(startup.Anomaly, loopID)

		result, err := protocol.Execute(ctx, completion.CommitRequest{
			LoopID:        loopID,
			CorrelationID: correlationID,
			FinalDiff:     commitSummary,
...
func buildCompletionTraceability(anomaly model.Anomaly, loopID string) (string, string, string) {
	storyID := strings.TrimSpace(anomaly.Metadata["prd_story_id"])
	prdSource := strings.TrimSpace(anomaly.Metadata["prd_source_ref"])
	sourceRef := strings.TrimSpace(anomaly.SourceRef)
	projectID := strings.TrimSpace(anomaly.Metadata["project_id"])

	subject := strings.TrimSpace(anomaly.Title)
	if subject == "" {
		subject = strings.TrimSpace(anomaly.Description)
	}
	if subject == "" {
		subject = "autonomous implementation update"
	}
	if storyID != "" {
		upperStory := strings.ToUpper(storyID)
		if !strings.Contains(strings.ToUpper(subject), upperStory) {
			subject = storyID + ": " + subject
		}
	}
	subject = collapseSingleLine(subject)

	trace := []string{subject, "", "Traceability:"}
	if prdSource != "" {
		trace = append(trace, "- PRD: "+prdSource)
	}
	if storyID != "" {
		trace = append(trace, "- Story: "+storyID)
	}
	if sourceRef != "" {
		trace = append(trace, "- SourceRef: "+sourceRef)
	}
	if projectID != "" {
		trace = append(trace, "- Project: "+projectID)
	}
	trace = append(trace, "- LoopID: "+loopID)
	if anomaly.CorrelationID != "" {
		trace = append(trace, "- CorrelationID: "+anomaly.CorrelationID)
	}

	prTitle := "feat(prd): " + subject
	if len(prTitle) > 120 {
		prTitle = prTitle[:120]
	...
	body := []string{
		"This pull request was autonomously generated by Smith.",
		"",
		"## Traceability",
		"- Loop ID: " + loopID,
	}
	if prdSource != "" {
		body = append(body, "- PRD: "+prdSource)
	}
	if storyID != "" {
		body = append(body, "- Story: "+storyID)
	}
	if sourceRef != "" {
		body = append(body, "- Source Ref: "+sourceRef)
	}
	if projectID != "" {
		body = append(body, "- Project: "+projectID)
	}
	if anomaly.CorrelationID != "" {
		body = append(body, "- Correlation ID: "+anomaly.CorrelationID)
	}

	return strings.Join(trace, "\n"), prTitle, strings.Join(body, "\n")
}

func collapseSingleLine(value string) string {
	...
	if len(value) > 88 {
		return strings.TrimSpace(value[:88])
	}
	return value
}
