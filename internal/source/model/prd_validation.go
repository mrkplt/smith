...
func (p *PRD) ValidateReport() PRDValidationReport {
	...
	for i, story := range p.Stories {
		p.validateStory(&report, story, i, storyIDs, &inProgressStories)
	}
	...
	for i, story := range p.Stories {
		p.validateStoryDependencies(&report, story, i, storyIDs)
	}
...
func (p *PRD) validateStory(report *PRDValidationReport, story PRDStory, index int, storyIDs map[string]int, inProgressStories *[]string) {
	storyPath := fmt.Sprintf("$.stories[%d]", index)
	storyID := strings.TrimSpace(story.ID)
	expectedID := fmt.Sprintf("US-%03d", index+1)
	...
	p.validateStoryAcceptance(report, story, storyPath, storyID)
...
func (p *PRD) validateStoryAcceptance(report *PRDValidationReport, story PRDStory, storyPath string, storyID string) {
...
func (p *PRD) validateStoryDependencies(report *PRDValidationReport, story PRDStory, index int, storyIDs map[string]int) {
	storyPath := fmt.Sprintf("$.stories[%d]", index)
	storyID := strings.TrimSpace(story.ID)
