...
expect_file "frontend/src/app.html" "Frontend SvelteKit entrypoint exists"
expect_file "frontend/deploy/nginx.conf" "Console nginx config exists"
expect_file "frontend/deploy/runtime-config.template.js" "Console runtime config template exists"
expect_file "frontend/deploy/30-smith-env.sh" "Console runtime env injection script exists"

expect_rg "SMITH_API_BASE_URL" "frontend/deploy/runtime-config.template.js" "Console runtime config references SMITH_API_BASE_URL"
expect_rg "location = /healthz" "frontend/deploy/nginx.conf" "Console liveness endpoint is defined"
expect_rg "location = /readyz" "frontend/deploy/nginx.conf" "Console readiness endpoint is defined"
...
expect_file ".github/workflows/ci.yml" "CI workflow exists"
expect_rg "image: core" ".github/workflows/ci.yml" "Workflow includes core image"
expect_rg "image: replica" ".github/workflows/ci.yml" "Workflow includes replica image"
expect_rg "image: console" ".github/workflows/ci.yml" "Workflow includes console image"
expect_rg "docker/core.Dockerfile" ".github/workflows/ci.yml" "Workflow references core Dockerfile"
expect_rg "docker/replica.Dockerfile" ".github/workflows/ci.yml" "Workflow references replica Dockerfile"
expect_rg "docker/console.Dockerfile" ".github/workflows/ci.yml" "Workflow references console Dockerfile"
expect_rg "docker/build-push-action" ".github/workflows/ci.yml" "Workflow uses docker build/push action"
expect_rg "aquasecurity/trivy-action" ".github/workflows/ci.yml" "Workflow includes vulnerability scanning gate"
expect_rg "anchore/sbom-action" ".github/workflows/ci.yml" "Workflow includes SBOM generation"
expect_rg "exit-code: '1'" ".github/workflows/ci.yml" "Scan failures are configured to fail CI"
expect_rg "type=sha" ".github/workflows/ci.yml" "Workflow publishes immutable SHA tags"

section "td-2d1f40 | CI matrix for single-loop and multi-loop e2e scenarios"
expect_file ".github/workflows/ci.yml" "CI workflow exists"
expect_rg "id: single-loop" ".github/workflows/ci.yml" "Workflow includes single-loop scenario"
expect_rg "id: multi-loop" ".github/workflows/ci.yml" "Workflow includes multi-loop scenario"
expect_rg "e2e-single-loop.sh" ".github/workflows/ci.yml" "Workflow runs single-loop e2e script"
expect_rg "e2e-concurrent-loops.sh" ".github/workflows/ci.yml" "Workflow runs multi-loop e2e script"
expect_rg "smith-e2e-single-loop-artifacts" ".github/workflows/ci.yml" "Workflow uploads single-loop artifacts"
expect_rg "smith-e2e-multi-loop-artifacts" ".github/workflows/ci.yml" "Workflow uploads multi-loop artifacts"
...
expect_rg "control/attach" "cmd/smith-api/main.go" "API exposes attach control endpoint"
expect_rg "control/detach" "cmd/smith-api/main.go" "API exposes detach control endpoint"
expect_rg "control/command" "cmd/smith-api/main.go" "API exposes interactive command endpoint"
...
expect_rg "handleLoopControlCommand" "cmd/smith-api/main.go" "Interactive command handler is defined"
expect_rg "loop detach" "internal/cli/control/main.go" "smith CLI exposes loop detach command"
expect_rg "loop command" "internal/cli/control/main.go" "smith CLI exposes loop command command"

section "td-5842f1 | operator console provider auth management"
expect_rg "Credential Label / ID|provider-credential-id" "frontend/src/lib/components/ProviderEditorDrawer.svelte" "Console captures provider credential label"
expect_rg "provider-api-key|API Key" "frontend/src/lib/components/ProviderEditorDrawer.svelte" "Console captures provider API key"
expect_rg "/v1/secrets" "frontend/src/lib/components/ProviderEditorDrawer.svelte" "Console upserts provider credential secret"
expect_rg "secret_ref" "cmd/smith-api/main.go" "API enforces secret_ref-backed provider credentials"
expect_rg "requires secret_ref for model inventory" "cmd/smith-api/main.go" "Model inventory requires secret_ref credentials"
expect_rg "type AuthStatus struct" "internal/source/provider/auth.go" "Provider auth status model remains available for compatibility"
expect_rg "TestAuthManagerConnectStatusRefreshDisconnect" "internal/source/provider/auth_test.go" "Provider auth lifecycle unit tests remain present"
...
expect_rg "Dockerfile Build Runtime Flags" "docs/loop-environment-profiles.md" "Loop environment docs cover dockerfile runtime flags"

section "td-fdcb47 | loop environment selection in smith CLI control package"
expect_rg "env-preset" "internal/cli/control/main.go" "smith CLI exposes environment preset flag"
expect_rg "--env-image-ref" "internal/cli/control/main.go" "smith CLI exposes environment image flag"
expect_rg "--env-docker-context" "internal/cli/control/main.go" "smith CLI exposes environment docker context flag"
expect_rg "buildEnvironmentPayload" "internal/cli/control/main.go" "smith CLI validates and builds environment payload"
expect_rg "environment source conflict" "internal/cli/control/main.go" "smith CLI rejects conflicting environment modes"

section "td-2558d6 | smith provider/project setup commands"
expect_rg "case \"provider\"" "internal/cli/control/main.go" "smith CLI exposes provider resource"
expect_rg "case \"project\"" "internal/cli/control/main.go" "smith CLI exposes project resource"
expect_rg "Commands: list, add, configure" "internal/cli/control/main.go" "smith provider/project commands include list/add/configure"
expect_rg "codex\|claude\|gemini" "internal/cli/control/main.go" "smith provider commands support codex/claude/gemini"
expect_rg "Configure provider first" "internal/cli/control/main.go" "smith help documents provider-before-project ordering"
expect_rg "\"status\":\s+\"ok\"|\"status\":\s+\"error\"" "internal/cli/control/main.go" "smith command outputs include machine-readable status envelope"
expect_rg "smith/internal/cli/control" "cmd/smith/main.go" "smith root command imports internal control package"
expect_rg "smith/internal/cli/replica" "cmd/smith/main.go" "smith root command imports internal replica package"

section "td-98b5f7 | provider-first onboarding flow in smith CLI control package"
expect_rg "ensureProviderFirstOnboarding" "internal/cli/control/main.go" "smith enforces provider-first onboarding precheck"
expect_rg "/v1/onboarding/readiness" "internal/cli/control/main.go" "smith checks API onboarding readiness contract"
expect_rg "provider_catalog" "internal/cli/control/main.go" "smith gates project/loop workflows on provider catalog readiness"
expect_rg "smith provider add --id codex-default --type codex --credential-id" "internal/cli/control/main.go" "smith CLI prints actionable provider setup suggestion"
expect_rg "TestProjectAddEnforcesProviderFirstOnboarding" "internal/cli/control/main_test.go" "smith provider-first project enforcement test exists"
expect_rg "TestLoopCreateEnforcesProviderFirstOnboarding" "internal/cli/control/main_test.go" "smith provider-first loop enforcement test exists"
...
expect_rg "e2e-environment-summary.txt" ".github/workflows/ci.yml" "CI summary includes environment e2e trace artifact"

section "td-d1c89c | smith skill mount configuration"
expect_rg "skillFlag" "internal/cli/control/main.go" "smith implements skill flag parser"
expect_rg "--skill" "internal/cli/control/main.go" "smith CLI exposes --skill flag"
expect_rg "provider defaults apply \(codex: /workspace/.agents/skills/<name>, claude: /workspace/.claude/skills/<name>\)" "internal/cli/control/main.go" "smith help documents provider-specific default mountpoints"
expect_rg "TestLoopCreateWithSkillFlags" "internal/cli/control/main_test.go" "smith skill flag payload test exists"
...
expect_rg "e2e-skill-mounts-summary.txt" ".github/workflows/ci.yml" "CI summary includes skill mount e2e artifact"
...
expect_rg "\\.smith/config.json" "Makefile" "Bootstrap prepares local smith config defaults"
...
expect_rg "non-vcluster-k3s-direct-gate" ".github/workflows/ci.yml" "Pre-release workflow includes direct k3s parity job"
expect_rg "smith-pre-release-parity-direct-k3s-artifacts" ".github/workflows/ci.yml" "Direct k3s parity artifacts are uploaded separately"

section "td-b77cdd | multi-arch image build support"
expect_rg "IMAGE_PLATFORMS" ".github/workflows/ci.yml" "Image workflow defines multi-arch platforms"
expect_rg "linux/amd64,linux/arm64" ".github/workflows/ci.yml" "Image workflow publishes amd64+arm64 manifest"
expect_rg "platforms: \\$\\{\\{ env.IMAGE_PLATFORMS \\}\\}" ".github/workflows/ci.yml" "Publish step uses multi-arch platforms"
...
expect_file ".github/workflows/ci.yml" "Pre-release gate workflow exists"
...
expect_rg "vcluster-system-gate" ".github/workflows/ci.yml" "Workflow includes vCluster gate job"
expect_rg "non-vcluster-parity-gate" ".github/workflows/ci.yml" "Workflow includes non-vCluster parity gate job"
expect_rg "pre-release-system-gate.sh vcluster" ".github/workflows/ci.yml" "Workflow runs vCluster gate profile"
expect_rg "pre-release-system-gate.sh parity" ".github/workflows/ci.yml" "Workflow runs parity gate profile"
